...
public abstract class Node {
    ...
    public Network network; // the current node's network information
    ...
    public static List<Message> runTrace(Node[] nodes, Trace trace, boolean batteryComputation, boolean dissemination, long seed) {
        ...
        LinkedList<ChatPair> chatPairs = trace.getChatPairs();
        ...
        for (long tick = startTime; tick < endTime; tick += sampleTime) {
            ...
            if (tick == chatPairsGenerationTime)  {
            	Message.updateActiveChatPairs(activeChatPairs, chatPairs, inChatPair, tick);
            ...
            for (int i = 0; i < contactCount; i++) {
                ...
                if (contact.getStart() <= tick && contact.getEnd() >= tick) {
                    ...
                    // run
                    observer.run(observed, tick, contactDuration, newContact, tick - startTime, sampleTime, contact.getType());   
                }
            }
     ...
     */
    protected void exchangeData(Node encounteredNode, long contactDuration, long currentTime, boolean contactType) {
        ...
        if (currentStats.getLastExchangeTime() + currentStats.getLastExchangeDuration() <= currentTime) {
            ...
            if (this.battery.canParticipate() && encounteredNode.battery.canParticipate()) {
                ...
                onDataExchange(encounteredNode, contactDuration, currentTime, contactType);
                encounteredNode.onDataExchange(this, contactDuration, currentTime, contactType);
            }
        }
    }
    ...
    abstract protected void onDataExchange(Node encounteredNode, long contactDuration, long currentTime, boolean contactType);
    ...
    protected int deliverDirectMessages(Node encounteredNode, boolean altruism, long contactDuration, 
    		long currentTime, boolean dissemination) {
    	return deliverDirectMessages(encounteredNode, altruism, contactDuration, currentTime, dissemination, false);
    }
     ...
     */
    protected int deliverDirectMessages(Node encounteredNode, boolean altruism, long contactDuration, 
    		long currentTime, boolean dissemination, boolean contactType) {
        List<Message> messagesForMe = new ArrayList<>();
        int maxMessages = network.computeMaxMessages(contactDuration, contactType);
        int totalMessages = 0;
